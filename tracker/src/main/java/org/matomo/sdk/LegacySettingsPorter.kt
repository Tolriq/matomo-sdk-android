...
class LegacySettingsPorter(matomo: Matomo) {
    ...
    fun port(tracker: Tracker) {
        ...
        val it: Iterator<Map.Entry<String, *>> = mLegacyPrefs.all.entries.iterator()
        while (it.hasNext()) {
            val oldEntry = it.next()
            if (oldEntry.key.startsWith("downloaded:")) {
                newSettings.edit().putBoolean(oldEntry.key, true).apply()
                mLegacyPrefs.edit().remove(oldEntry.key).apply()
            }
        }
    }
